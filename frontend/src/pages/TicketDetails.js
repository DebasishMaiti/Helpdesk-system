...
function TicketDetail() {
    ...
    const fetchTicket = async () => {
        try {
            const res = await axios.get(`https://helpdesk-system-cety.vercel.app/api/ticket/${id}`, {
                headers: {
                    'Authorization': token,
                }
            });
    ...
    const handleNoteSubmit = async (e) => {
        ...
        try {
            await axios.post(`https://helpdesk-system-cety.vercel.app/api/ticket/${id}/notes`, formData, {
                headers: {
                    'Authorization': token,
    ...
    const deleteNote = async (noteId) => {
        ...
        try {
            await axios.delete(`https://helpdesk-system-cety.vercel.app/api/notes/${noteId}`, {
                                ...
                                {note.attachments.map((f, i) => (
                                    <li key={i}>
                                        {isImage(f) ? (
                                            <div>
                                                <img
                                                    src={`https://helpdesk-system-cety.vercel.app/Uploads/${f}`}
                                                    alt={f}
                                                    className="note-img"
                                                />
                                                <a href={`https://helpdesk-system-cety.vercel.app/Uploads/${f}`} target="_blank" rel="noreferrer">
                                                    View Full Image
                                                </a>
                                            </div>
                                        ) : (
